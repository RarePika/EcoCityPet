...
public class DBOUpdater
{
    ...
    private DBOUpdater.UpdateResult result = DBOUpdater.UpdateResult.SUCCESS; // Used for determining the outcome of the update process
     ...
     */
    public DBOUpdater(Plugin plugin, int id, File file, UpdateType type, boolean announce)
    {
        this.plugin = plugin;
        this.type = type;
        ...
        try
        {
            this.url = new URL(DBOUpdater.HOST + DBOUpdater.QUERY + id);
        }
     ...
     */
    public DBOUpdater.UpdateResult getResult()
    {
        this.waitForThread();
        return this.result;
    }
    ...
    private void saveFile(File folder, String file, String u)
    {
        ...
        try
        {
            ...
            final byte[] data = new byte[DBOUpdater.BYTE_SIZE];
            int count;
            if (this.announce)
            {
            ...
            long downloaded = 0;
            while ((count = in.read(data, 0, DBOUpdater.BYTE_SIZE)) != -1)
            {
                downloaded += count;
        ...
        catch (final Exception ex)
        {
            ...
            this.result = DBOUpdater.UpdateResult.FAIL_DOWNLOAD;
        }
        finally
        {
            try
        ...
        try
        {
            ...
            while (e.hasMoreElements())
            {
                ...
                else
                {
                    ...
                    int b;
                    final byte buffer[] = new byte[DBOUpdater.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, DBOUpdater.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, DBOUpdater.BYTE_SIZE)) != -1)
                    {
        ...
        if (this.type != UpdateType.NO_VERSION_CHECK)
        {
            ...
            if (title.split(" v").length == 2)
            {
                ...
                if (this.hasTag(version) || version.equalsIgnoreCase(remoteVersion))
                {
                    ...
                    this.result = DBOUpdater.UpdateResult.NO_UPDATE;
                    return false;
                }
            }
            else
            {
                ...
                this.result = DBOUpdater.UpdateResult.FAIL_NOVERSION;
                return false;
            }
        }
        return true;
    ...
    {
        for (final String string : DBOUpdater.NO_UPDATE_TAG)
        {
    ...
    private boolean read()
    {
        try
        {
            ...
            this.versionName = (String) ((JSONObject) array.get(array.size() - 1)).get(DBOUpdater.TITLE_VALUE);
            this.versionLink = (String) ((JSONObject) array.get(array.size() - 1)).get(DBOUpdater.LINK_VALUE);
            this.versionType = (String) ((JSONObject) array.get(array.size() - 1)).get(DBOUpdater.TYPE_VALUE);
            this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1)).get(DBOUpdater.VERSION_VALUE);
    ...
    private class UpdateRunnable implements Runnable
    {
        ...
        public void run()
        {
            if (DBOUpdater.this.url != null)
            {
                // Obtain the results of the project's file feed
                if (DBOUpdater.this.read())
                {
                    if (DBOUpdater.this.versionCheck(DBOUpdater.this.versionName))
                    {
                        if ((DBOUpdater.this.versionLink != null) && (DBOUpdater.this.type != UpdateType.NO_DOWNLOAD))
                        {
                            String name = DBOUpdater.this.file.getName();
                            // If it's a zip file, it shouldn't be downloaded as the plugin's name
                            if (DBOUpdater.this.versionLink.endsWith(".zip"))
                            {
                                final String[] split = DBOUpdater.this.versionLink.split("/");
                                name = split[split.length - 1];
                            }
                            DBOUpdater.this.saveFile(new File(DBOUpdater.this.plugin.getDataFolder().getParent(), DBOUpdater.this.updateFolder), name, DBOUpdater.this.versionLink);
                        }
                        else
                        {
                            DBOUpdater.this.result = UpdateResult.UPDATE_AVAILABLE;
                        }
                    }
                }
            }
