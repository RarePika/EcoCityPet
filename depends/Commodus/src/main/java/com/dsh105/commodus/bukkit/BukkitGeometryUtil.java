...
public class BukkitGeometryUtil
{
    ...
    {
        return PositionContainer.toBukkit(GeometryUtil.circle(PositionContainer.from(origin), radius, height, hollow, sphere, includeAir));
    }
     ...
     * @param <T>             Rntity restriction to place on the search
    ...
    public static <T extends Entity> List<T> getNearbyEntities(Class<T> typeRestriction, Location origin, int range)
    {
        ...
        {
            PositionContainer container = PositionContainer.from(entity.getLocation());
    ...
    {
        return positionContainer.toBukkit().getBlock().getType() == Material.AIR;
    }
}